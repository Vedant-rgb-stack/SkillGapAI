...
def generate_pdf():
    ...
    pdf.cell(0, 10, "SkillGapAI - Skill Gap Report", ln=True, align="C")
    pdf.ln(8)

    pdf.cell(0, 10, f"Overall Match: {overall_match}%", ln=True)
    pdf.cell(0, 10, f"Matched Skills: {matched}", ln=True)
    pdf.cell(0, 10, f"Missing Skills: {missing}", ln=True)

    pdf.ln(8)
    pdf.cell(0, 10, "Skill Comparison:", ln=True)

    for _, row in skills_df.iterrows():
        pdf.multi_cell(
            0, 8,
            f"{row['Skill']} - Resume: {row['Resume Score']}%, "
            f"Job: {row['Job Requirement Score']}%"
        )
